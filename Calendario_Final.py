...
ruta_json = "Calendario_Final\\Especies.json" # Determinamos la ruta donde se encuentra el archivo.

try: # Un try que encierra el programa para manejar excepciones de error de lectura del archivo Especies.json. 
    with open(ruta_json, 'r', encoding='utf-8') as archivo: # Abrimos el archivo en modo lectura y utf-8 para "ñ".
        especies = json.load(archivo) # cargamos el contenido a la variable "especies".

    def limpiarPantalla(): #Funcion para limpiar pantalla.
        os.system('cls' if os.name == 'nt' else 'clear')
        return

    def buscar_por_especie(busqueda_especie): #Funcion para buscar especies.
        '''En esta funcion:
        Se carga el archivo Especies.json en una variable,
        Iteramos dentro del diccionario que contiene para buscar la especie requerida,
        y presentamos la informacion formateada'''
        limpiarPantalla()
    ...
    def buscar_por_estacion(estacion): #Funcion para buscar especies por estacion.
        '''En esta funcion:
        Se carga el archivo Especies.json en una variable
        Iteramos dentro de un valor especifico dentro de las claves del diccionario: "Momento de siembra", 
        para buscar la estacion requerida,
        y presentamos la lista de especies que pueden sembrarse en ella,  formateada,
        ofreciendo ademas ver los detalles de alguna de las especies listadas.'''
        limpiarPantalla()
        print(colorama.Fore.RED + "*" * 50)
    ...
    def Agregar_especie(): # Funcion para agregar una nueva especie.
        '''En esta funcion:
        el usuario puede cargar una especie,
        (se crea un diccionario con los parametros de especie como clave)
        y se solicita al usuario que ingrese sus valores,
        luego se abre el archivo en modo escritura y se vuelca al final del diccionario que este contiene.'''
        limpiarPantalla()
        print(colorama.Fore.RED + "*" * 50)
    ...
    def Modificar_especie(nombre_especie): #Funcion para modificar una especie existente.
        '''En esta funcion:
        Se carga el archivo Especies.json en una variable
        y se itera sobre el diccionario que contiene buscando la especie requerida,
        y en ella se reemplazan los valores o se mantienen segun el usuario elija,
        luego se vuelca en el archivo nuevamente '''
        limpiarPantalla()
        print(colorama.Fore.RED + "*" * 50)
    ...
    def Eliminar_especie(): # Funcion para eliminar una especie existente.
        '''En esta funcion:
        Se carga el archivo Especies.json en una variable
        y se itera sobre el diccionario que contiene buscando la especie requerida,
        y se elimina del diccionario '''
        limpiarPantalla()
        print(colorama.Fore.RED + "*" * 50)
    ...
    def Listar_especies(): #Funcion para listar especies existentes.
        '''En esta funcion:
    Se carga el archivo Especies.json en una variable
    y se itera sobre el diccionario
    para extraer por separado las claves y valores y mostrarlas formateadas,
    ademas se ofrece ver los detalles de alguna de las especies listadas.'''
        limpiarPantalla()
        print(colorama.Fore.RED + "*" * 50)
        ...
        try:
            ...
            print(colorama.Fore.YELLOW + "Especies en el Calendario:")
            for nombre_especie in especies.keys():
                print(f"- {colorama.Fore.GREEN + nombre_especie}")
            
            # Preguntar al usuario si desea ver detalles de alguna especie.
            ver_detalles = input(colorama.Fore.YELLOW + "¿Quieres ver los detalles de alguna especie? (s/n): ").strip().lower()
            if ver_detalles == "s":
    ...
    def calcular_dia_de_cosecha(nombre_especie, fecha_siembra): # Funcion para calcular el dia de cosecha.
        '''En esta funcion:
        Se carga el archivo Especies.json en una variable
        y se itera sobre el diccionario que contiene buscando la especie requerida,
        luego se toman aislados los valores numericos que indican "desde" y "hasta"
        en las claves "momento de cosecha" y "transplante", y tomando en cuenta la fecha de siembra
        ingresada por el usuario, se calcula el dia de cosecha y emite una alerta si
        debe transplantarse antes '''
        limpiarPantalla()
        print(colorama.Fore.RED + "*" * 50)
            ...
            if nombre_especie in especies:
                ...
                if "a" in tiempo_cosecha.lower():  # Verifica si hay un rango de días
                    try:
                        ...
                        print(colorama.Fore.RESET + f"El rango estimado de cosecha para la especie '{colorama.Fore.GREEN + nombre_especie}' es desde {colorama.Fore.GREEN + fecha_cosecha_min.strftime('%d/%m/%Y')} hasta {colorama.Fore.GREEN + fecha_cosecha_max.strftime('%d/%m/%Y')}.")
                    except ValueError:
                        return None
                else:
                ...
                if tiempo_trasplante == "n/a":
                        print(colorama.Fore.YELLOW + " No requiere transplante.")
                        if tiempo_trasplante != "n/a":
    ...
    def menu(): #Funcion para general el menu.
        while True:
            limpiarPantalla()
            ...
            if opcion == "1":
                busqueda_especie = input(colorama.Fore.YELLOW + "Ingrese el nombre de la especie que desea buscar: ")
                buscar_por_especie(busqueda_especie)
            elif opcion == "2":
                estacion = input(colorama.Fore.YELLOW + "Ingrese la estación (primavera, verano, otoño, invierno): ")
                buscar_por_estacion(estacion)
            elif opcion == "3":
                Agregar_especie()
            elif opcion == "4":
                nombre_especie = input(colorama.Fore.YELLOW + "Ingrese el nombre de la especie que desea modificar: ")
                ...
                nombre_especie = input(colorama.Fore.YELLOW + "Ingrese el nombre de la especie: ")
                fecha_siembra_str = input(colorama.Fore.YELLOW + "Ingrese la fecha de siembra (formato: dd/mm/aaaa): ")
                try:
